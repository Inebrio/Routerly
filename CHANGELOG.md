...
- OpenAI's default-reasoning chat models (the `gpt-5.6` family — Sol, Terra, Luna) no longer 400 on any request that includes function tools. These models reason by default on `/v1/chat/completions` unless told `reasoning_effort: 'none'`, even when the field is never sent — Routerly now catches that specific rejection and retries once with `reasoning_effort: 'none'` forced on, rather than hardcoding a model list that changes release to release.
