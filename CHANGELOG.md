...
- `test/setup_alibaba_test.py` no longer hardcodes the local admin password and a DashScope API key — both were committed to history and pushed to the remote; the key has been revoked and the leaked history purged from every branch except `main`/`develop` (blocked by branch protection, fixed here via normal PR instead). The script now reads `ROUTERLY_SMOKE_EMAIL`, `ROUTERLY_SMOKE_PASSWORD`, and `DASHSCOPE_KEY` from the environment and refuses to run if any are unset.
