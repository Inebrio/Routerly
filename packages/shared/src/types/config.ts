...
/**
 * Name a connection identifies itself by everywhere it is referenced: the models list,
 * the connection picker on the model form, the CLI table. Two connections sharing a name
 * make those lists unreadable, so the name is unique and is generated by default.
 *
 * The generated name is a slug of what the connection connects to (the upstream name for a
 * custom connection, the provider id otherwise), with a numeric suffix when that slug is
 * already taken. A name typed by hand is kept as typed; it only has to be free.
 *
...
/** True when `label` is already worn by one of `taken`, ignoring case and surrounding space. */
