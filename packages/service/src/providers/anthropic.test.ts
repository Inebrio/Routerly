...
describe('AnthropicAdapter prompt caching (#97)', () => {
  it('auto mode: string system prompt is converted to array with cache_control on last block', async () => {
    mockCreate.mockResolvedValue({
      id: 'msg-pc-auto-sys', model: 'claude-3-haiku',
      content: [{ type: 'text', text: 'ok' }],
      stop_reason: 'end_turn',
      usage: { input_tokens: 5, output_tokens: 2, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 },
    })
    ...
    }, makeModel({ promptCaching: 'auto' } as any))

    const callParams = mockCreate.mock.calls[0]![0]
    expect(Array.isArray(callParams.system)).toBe(true)
    expect(callParams.system[callParams.system.length - 1].cache_control).toEqual({ type: 'ephemeral' })
  })

  it('auto mode: penultimate user message gets cache_control on its last content block', async () => {
    mockCreate.mockResolvedValue({
      id: 'msg-pc-auto-msg', model: 'claude-3-haiku',
      content: [{ type: 'text', text: 'ok' }],
      stop_reason: 'end_turn',
      usage: { input_tokens: 5, output_tokens: 2, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 },
    })
    ...
    const callParams = mockCreate.mock.calls[0]![0]
    const userMessages = callParams.messages.filter((m: any) => m.role === 'user')
    // penultimate user message (index -2) should have cache_control injected
    const penultimate = userMessages[userMessages.length - 2]
    // string content gets converted to array with cache_control
    const lastBlock = Array.isArray(penultimate.content)
      ? penultimate.content[penultimate.content.length - 1]
      : penultimate.content
    expect(lastBlock.cache_control).toEqual({ type: 'ephemeral' })
  })

  it('disabled mode: strips cache_control from system blocks and message content blocks', async () => {
    mockCreate.mockResolvedValue({
      id: 'msg-pc-disabled', model: 'claude-3-haiku',
      content: [{ type: 'text', text: 'ok' }],
      stop_reason: 'end_turn',
      usage: { input_tokens: 5, output_tokens: 2, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 },
    })
    ...
    await adapter.chatCompletion({
      model: 'auto',
      messages: [
        // system with cache_control via array content
        { role: 'system', content: [{ type: 'text', text: 'Be helpful.', cache_control: { type: 'ephemeral' } }] },
        // user with cache_control on a content block
        { role: 'user', content: [{ type: 'text', text: 'Hi', cache_control: { type: 'ephemeral' } }] },
      ],
    } as any, makeModel({ promptCaching: 'disabled' } as any))

    const callParams = mockCreate.mock.calls[0]![0]
    expect(callParams.system[0].cache_control).toBeUndefined()
    expect(callParams.messages[0].content[0].cache_control).toBeUndefined()
  })

  it('passthrough (undefined): no cache_control modifications to params', async () => {
    mockCreate.mockResolvedValue({
      id: 'msg-pc-passthrough', model: 'claude-3-haiku',
      content: [{ type: 'text', text: 'ok' }],
      stop_reason: 'end_turn',
      usage: { input_tokens: 5, output_tokens: 2, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 },
    })
    ...
    }, makeModel())  // no promptCaching field → applyPromptCachingMode not called

    const callParams = mockCreate.mock.calls[0]![0]
    // system stays as plain string — not wrapped in array
    expect(typeof callParams.system).toBe('string')
    expect(callParams.system).toBe('You are helpful.')
  })
})
