...
describe('GET /api/agent-policies', () => {
  it('returns the project agentPolicies', async () => {
    setupAdminAuth()
    const project = { id: 'p1', name: 'P', tokens: [], members: [], models: [], agentPolicies: [{ name: 'fast', models: ['m1'] }] }
    mockReadConfig.mockImplementation(async (t: string) => {
      if (t === 'users') return [adminUser]
      if (t === 'roles') return []
      if (t === 'projects') return [project]
    ...
    const app = await buildApp()
    const res = await app.inject({ method: 'GET', url: '/api/agent-policies?projectId=p1', headers: adminAuthHeaders() })
    await app.close()
    expect(res.statusCode).toBe(200)
    expect(JSON.parse(res.body)).toEqual([{ name: 'fast', models: ['m1'] }])
  })

  it('returns empty array when project has no agentPolicies', async () => {
    setupAdminAuth()
    const project = { id: 'p1', name: 'P', tokens: [], members: [], models: [] }
    mockReadConfig.mockImplementation(async (t: string) => {
      if (t === 'users') return [adminUser]
      if (t === 'roles') return []
      if (t === 'projects') return [project]
  ...
  it('returns 400 without projectId', async () => {
    setupAdminAuth()
    const app = await buildApp()
    const res = await app.inject({ method: 'GET', url: '/api/agent-policies', headers: adminAuthHeaders() })
    await app.close()
    expect(res.statusCode).toBe(400)
  })
  ...
  it('returns 404 for unknown project', async () => {
    ...
    const app = await buildApp()
    const res = await app.inject({ method: 'GET', url: '/api/agent-policies?projectId=nope', headers: adminAuthHeaders() })
    await app.close()
    expect(res.statusCode).toBe(404)
  })

  it('returns 403 without project:read', async () => {
    const noPerm = { id: 'np', email: 'np@x.com', passwordHash: 'h', roleId: 'norole', projectIds: [] }
    mockVerifyToken.mockReturnValue({ sub: 'np' } as any)
    mockReadConfig.mockImplementation(async (t: string) => {
      if (t === 'users') return [noPerm]
      if (t === 'roles') return [{ id: 'norole', name: 'No', permissions: [] }]
      return []
    })
    const app = await buildApp()
    const res = await app.inject({ method: 'GET', url: '/api/agent-policies?projectId=p1', headers: adminAuthHeaders() })
...
describe('PUT /api/agent-policies', () => {
  it('persists validated policies', async () => {
    setupAdminAuth()
    const project = { id: 'p1', name: 'P', tokens: [], members: [], models: [] }
    mockReadConfig.mockImplementation(async (t: string) => {
      if (t === 'users') return [adminUser]
    ...
    const app = await buildApp()
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [{ name: 'fast', models: ['m1', 'm2'], maxCostUsd: 0.5, maxLatencyMs: 2000 }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(200)
    const written = mockWriteConfig.mock.calls.find(c => c[0] === 'projects')![1] as any[]
    expect(written[0].agentPolicies).toEqual([{ name: 'fast', models: ['m1', 'm2'], maxCostUsd: 0.5, maxLatencyMs: 2000 }])
  })

  it('strips unknown extras and omits absent optional fields', async () => {
    setupAdminAuth()
    const project = { id: 'p1', name: 'P', tokens: [], members: [], models: [] }
    mockReadConfig.mockImplementation(async (t: string) => {
      if (t === 'users') return [adminUser]
    ...
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [{ name: ' slow ', models: ['m1'], bogus: 1 }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(200)
    expect(JSON.parse(res.body)).toEqual([{ name: 'slow', models: ['m1'] }])
  })

  it('returns 400 for empty policy name', async () => {
    setupAdminAuth()
    ...
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [{ name: '', models: ['m1'] }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(400)
  })

  it('returns 400 for duplicate policy names', async () => {
    setupAdminAuth()
    mockReadConfig.mockImplementation(async (t: string) => (t === 'users' ? [adminUser] : []))
    const app = await buildApp()
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [{ name: 'a', models: ['m1'] }, { name: 'a', models: ['m2'] }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(400)
  })
  ...
  it('returns 400 for empty models array', async () => {
    ...
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [{ name: 'a', models: [] }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(400)
  })

  it('returns 400 for negative maxCostUsd', async () => {
    setupAdminAuth()
    mockReadConfig.mockImplementation(async (t: string) => (t === 'users' ? [adminUser] : []))
    const app = await buildApp()
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [{ name: 'a', models: ['m1'], maxCostUsd: -1 }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(400)
  })

  it('returns 400 when agentPolicies is not an array', async () => {
    setupAdminAuth()
    mockReadConfig.mockImplementation(async (t: string) => (t === 'users' ? [adminUser] : []))
    const app = await buildApp()
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: 'nope' }),
    })
    await app.close()
    expect(res.statusCode).toBe(400)
  })
  ...
  it('returns 400 without projectId', async () => {
    ...
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ agentPolicies: [] }),
    })
    await app.close()
    expect(res.statusCode).toBe(400)
  })
  ...
  it('returns 404 for unknown project', async () => {
    ...
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'nope', agentPolicies: [{ name: 'a', models: ['m1'] }] }),
    })
    await app.close()
    expect(res.statusCode).toBe(404)
  })
  ...
  it('returns 403 without project:write', async () => {
    const viewer = { id: 'v', email: 'v@x.com', passwordHash: 'h', roleId: 'viewer', projectIds: [] }
    mockVerifyToken.mockReturnValue({ sub: 'v' } as any)
    mockReadConfig.mockImplementation(async (t: string) => {
    ...
    const res = await app.inject({
      method: 'PUT', url: '/api/agent-policies',
      headers: { ...adminAuthHeaders(), 'content-type': 'application/json' },
      payload: JSON.stringify({ projectId: 'p1', agentPolicies: [] }),
    })
    await app.close()
    expect(res.statusCode).toBe(403)
  })
