 ...
 * fairness/performance/budget-remaining). Every other type (`cheapest`, `capability`,
 * `context`, `llm`, `semantic-intent`, `model-preference`) picks among a *pool of
 * models* — an Orchestrator has no models of its own to score, so those types are
 * rejected at write time instead of being silently stored and ignored.
 */
export const ORCHESTRATOR_POLICY_TYPES: RoutingPolicy['type'][] = ['health', 'rate-limit', 'fairness', 'performance', 'budget-remaining'];
