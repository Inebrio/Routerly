...
beforeEach(() => {
  ...
  mockReadConfig.mockImplementation(async (type: string) => {
    if (type === 'users') return [{ id: 'u1', email: 'u1@example.com', roleId: 'member', projectIds: ['p1'] }]
...
describe('requiredPermission', () => {
  it('maps each event family to the permission its subject is read through', () => {
    ...
    expect(requiredPermission('config.project_deleted')).toBe('project:read')
...
describe('loadInboxScope', () => {
  it('scopes a user to the projects they belong to', async () => {
    const scope = await loadInboxScope('u1')
    expect([...scope.known]).toEqual(['p1', 'p2'])
    expect([...scope.mine]).toEqual(['p1'])
  })

  it('gives an unknown user no project at all', async () => {
...
describe('isVisibleToUser', () => {
  ...
  it('hides an event about a project the user cannot reach', async () => {
    const scope = await loadInboxScope('u1')
    expect(isVisibleToUser(item({ details: { projectId: 'p2' } }), 'u1', [], scope)).toBe(false)
    expect(isVisibleToUser(item({ details: { projectId: 'p1' } }), 'u1', [], scope)).toBe(true)
  })

  it('keeps an event about a project that is gone', async () => {
    const scope = await loadInboxScope('u1')
    expect(isVisibleToUser(item({ details: { projectId: 'p9' } }), 'u1', [], scope)).toBe(true)
  })

  it('ignores a non-string projectId', async () => {
    const scope = await loadInboxScope('u1')
    expect(isVisibleToUser(item({ details: { projectId: 42 } }), 'u1', [], scope)).toBe(true)
  })
