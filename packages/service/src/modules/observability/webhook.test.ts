...
const snapshot = {
  agg: {
    requests: new Map([['k', { labels: { router: 'P', model: 'M', provider: 'openai', status: 'success' }, value: 4 }]]),
    tokens: new Map(),
    cost: new Map(),
    durations: new Map(),
  },
...
describe('pushWebhook', () => {
  ...
  it('includes token, cost and duration fields when maps are non-empty', async () => {
    ...
    const richSnapshot = {
      agg: {
        requests: new Map([['r', { labels: { router: 'P', model: 'M', provider: 'openai', status: 'success' }, value: 4 }]]),
