...
const response: Processor<ProxyContext> = {
  ...
  async run(ctx) {
    ...
    if (ctx.result?.kind === 'stream') {
      ...
      // the doubly-wrapped iterator. The helper checks guardrails and sets ctx.blockedBy itself once
      // the stream ends; it does not emit trace entries for that deferred check (helper predates this
      // module, out of scope to change here).
