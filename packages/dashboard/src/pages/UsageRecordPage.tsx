...
/**
 * Deep dive: the entries of one pipeline phase, in the order the modules emitted
 * them, each stamped with the module that spoke and how far into the request it was.
 */
function PhaseSection({ phase, entries, startedAt }: { phase: string; entries: TraceEntry[]; startedAt: number }) {
...
function TracePanel({ entries }: { entries: TraceEntry[] }) {
  ...
  const phases = [...new Set(filteredEntries.map(e => e.phase).filter(Boolean))] as string[];
  if (phases.length > 0) {
    const startedAt = filteredEntries.find(e => e.at != null)?.at ?? 0;
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {phases.map(phase => (
          <PhaseSection key={phase} phase={phase} startedAt={startedAt} entries={filteredEntries.filter(e => e.phase === phase)} />
        ))}
      </div>
    );
  }
