...
export type RuleWithId = GuardrailRule & { _id: string };

export const RULE_TYPE_LABELS: Record<GuardrailRuleType, string> = {
  regex:      'Regex',
  semantic:   'Semantic',
  topic:      'Topic',
...
export function normalizeGuardActions<T extends GuardrailRule>(r: T): T {
...
/** Blocking validation for a rule set: regex syntax and required moderation instructions. */
export function validateSecurityRules(rules: RuleWithId[]): {
  regexErrorsByRule: Record<string, number[]>;
  moderationErrorIds: Set<string>;
} {
  ...
  return { regexErrorsByRule, moderationErrorIds };
}

interface SecurityRulesEditorProps {
  rules: RuleWithId[];
  setRules: React.Dispatch<React.SetStateAction<RuleWithId[]>>;
  piiPolicies: PiiPolicy[];
  setPiiPolicies: React.Dispatch<React.SetStateAction<PiiPolicy[]>>;
}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {rules.map(r => (
              <RuleCard
                key={r._id}
                rule={r}
                onChange={updated => setRules(prev => prev.map(x => x._id === r._id ? updated : x))}
                onDelete={() => setRules(prev => prev.filter(x => x._id !== r._id))}
