...
```bash
routerly clients configure zed --router my-api
```

Prints the block below with a real router token, mints one if you do not
...
a specific model registered in your router.
...
- **Agent panel**: open the assistant, **Settings → Providers → Routerly**,
  paste the router token. Zed stores it in the system keychain.
- **Environment**: export `OPENAI_API_KEY=sk-rt-YOUR_ROUTER_TOKEN` before
...
tokens. That token is a personal MCP token (`sk-rt-mcp-…`), not a router
