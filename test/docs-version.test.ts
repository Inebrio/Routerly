...
describe('RA-09 — happy path', () => {
  it(
    'AC1/AC2 — cuts a full copy of docs/, puts the new version first, leaves docusaurus.config.ts untouched',
    () => {
      const f = fixture({ initialVersions: ['0.1.0'] })
      const configBefore = readConfigRaw(f)
      ...
      // RC-2/task 7 — the script no longer touches docusaurus.config.ts at
      // all (no lastVersion to rewrite; Docusaurus's unset default already
