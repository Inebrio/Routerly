...
description: Run a Tier 1 or Tier 2 feature end to end — feature level, story level, parallelism, integration, and retrospective. Use when you are running a multi-agent feature per CLAUDE.md's Workflow tiers, not for Tier 0 inline work.
---

# Feature lifecycle

The mechanics for running a Tier 1 or Tier 2 feature, once the tier is picked
per `CLAUDE.md`'s Workflow section. See `story-lifecycle` for what a single
